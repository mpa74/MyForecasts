...
1. In `MyForecasts.Presentation.csproj`, replace the `<TargetFrameworks ... />` elements with the `<TargetFrameworks ... />` elements from `MyForecasts.csproj`; both projects should have the same target frameworks
...
3. **Rebuild** the `MyForecasts.Windows` project and run it
...
You can use `New-View.ps1` (located in the `MyForecasts.Presentation` project folder) to quickly add new views - including (view)model, if applicable. See `New-View.ps1` for tips and parameters.
